...
def convert_speech_to_text(audio_bytes):
    ...
    try:
        text = recognizer.recognize_google(audio, language='zh')  # 使用Google语音识别引擎，语言为英文
        return text
    ...
    with chat_tab:
      ...
      #   print("************输出*******",ResultText)
        ...
        def clear_text():
            ...
            # session.transcript.append(f"Candidate: {ResultText}")
        ...
        if audio_bytes:
            ...
            global ResultText
            ResultText = convert_speech_to_text(audio_bytes) 

            print("转换后的文本:", ResultText)
            st.write(ResultText)
